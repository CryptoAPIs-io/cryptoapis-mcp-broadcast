...
const CRYPTOAPIS_SERVER_INFO = {
    name: "cryptoapis-broadcast",
    version: "0.1.0",
    title: "CryptoAPIs Broadcast",
...
export async function startBroadcastServer(opts: StartOptions) {
    ...
    if (opts.transport === "stdio") {
        ...
        logger.logInfo("cryptoapis-broadcast MCP running (stdio)");
        return;
    }
    ...
    app.listen(port, host, () => {
        logger.logInfo(`cryptoapis-broadcast MCP running (http) at http://${host}:${port}${path}`);
